...
/** Annotation stored in core store — no Konva shapes (media-agnostic). */
export interface AnnotationCore extends AnnotationsEntity {
...
export class ReviewCoreStore {
  ...
  constructor(private readonly mediaType: MediaType) {
    makeObservable(this, {
      assetId: observable,
  ...
  get filteredAnnotations(): AnnotationCore[] {
  ...
  get openCount(): number {
    ...
    return raw !== undefined
      ? parseInt(raw as unknown as string, 10) || 0
  ...
  async init(assetId: string, versionNumber?: number): Promise<void> {
    ...
    // Phase 1: asset-level loads (no image data — that's ImagePlayerStore's job)
    await Promise.all([
    ...
    // Phase 2: version-dependent loads
    await Promise.all([
  ...
  async fetchAssetDetail(assetId: string): Promise<void> {
    try {
      ...
      if (response?.isSuccessful) {
        runInAction(() => { this.assetDetail = response.data ?? null; });
      }
    } catch { /* non-blocking */ }
  }
  ...
  async fetchVersionList(assetId: string): Promise<void> {
    ...
    try {
      ...
      if (response?.isSuccessful) {
        runInAction(() => { this.versions = response.data?.data ?? []; });
      }
    } catch { /* non-blocking */ }
    finally {
  ...
  /** Creates a root-level annotation. Receives pre-converted region (caller handles coordinate transform). */
  ...
  handleSseAnnotationEvent(event: AnnotationSseEvent): void {
    ...
    switch (eventType) {
      ...
      case 'UPDATED': {
        ...
        if (idx !== -1) {
          const prev = this.annotations[idx];
          // Store raw region only — konvaShapes recomputed via ImagePlayerStore computed
          this.annotations[idx] = {
            ...prev,
            ...annotation,
            replies: prev.replies,
  ...
  private _wrapAnnotation(a: AnnotationsEntity, authorName?: string): AnnotationCore {
    return {
      ...a,
